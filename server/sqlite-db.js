...
function createTable() {
  dbConfig.sqlite = sqlite3.verbose();
  // var db = new sqlite.Database(':memory:');
  var db = new dbConfig.sqlite.Database('test.db');
  if (!db) {
    db.serialize(function () {
      db.run('CREATE TABLE height_formula (min_height integer, max_height integer, formula text)');
      const stmt = db.prepare('INSERT INTO height_formula(min_height, max_height, formula) VALUES (?, ?, ?)');
      for (let i = 0; i < forumla.length; i++) {
        const f = forumla[i];
        stmt.run(f.min_height, f.max_height, f.formula);
      }
      stmt.finalize();

      db.each('SELECT rowid AS id, min_height, max_height, formula FROM height_formula', function (err, row) {
        console.log(row.id + ': ' + row.max_height + '-' + row.formula);
      });
    });
  }
...
function getFormula(height) {
  var sqlite = sqlite3.verbose();
  var db = new sqlite.Database('test.db', sqlite3.OPEN_READONLY);
  var result;
  // var getResult = db.get(`SELECT rowid AS id, min_height, max_height, formula FROM height_formula where min_height <= ${height} and max_height >= ${height}`)
  db.serialize(function () {
    db.each('SELECT rowid AS id, min_height, max_height, formula FROM height_formula', function (err, row) {
      if (height >= row.min_height && (row.max_height == null || height <= row.max_height)) {
        console.info(row.formula);
        result = row;
      }
    });
    return result;
